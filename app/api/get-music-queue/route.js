...
export async function POST(req) {
    ...
    try{
        ...
        const {data,error} = await supabase.from('musicqueuedata').select('*').eq('song_room_id',RoomID);

        if(error){
            return NextResponse.json({'message':'Error Fetching Music Queue','status':500});
        }
